...
mechdog.set_servo(9,_arm_pwm,10)
mechdog.set_servo(10,_claw_pwm,10)
time.sleep(1)
  ...
  while True:
    if ble.is_connected():
      if ble.contains_data("CMD"):
        ...
        if (_COMMAND==7):
          _DATA = int(_REC_PARSE_VALUE[1])
          if (_DATA==3): #robot arm
            _DATA = int(_REC_PARSE_VALUE[2])
            if(_DATA == 1):
              if(_arm_pwm < 1980):
                _arm_pwm += 30
                mechdog.set_servo(9,_arm_pwm,100)
            elif(_DATA == -1):
              if(_arm_pwm > 1350):
                _arm_pwm -= 30
                mechdog.set_servo(9,_arm_pwm,100)
          if (_DATA==4): #gripper
            ...
            if(_DATA == 1):
              if(_claw_pwm < 1850):
                _claw_pwm += 30
                mechdog.set_servo(10,_claw_pwm,100)
            elif(_DATA == -1):
              if(_claw_pwm > 1450):
                _claw_pwm -= 30
                mechdog.set_servo(10,_claw_pwm,100)
          ...
          if (_DATA==5): #gripper
              _arm_pwm = 1980
              _claw_pwm = 1800
              mechdog.set_servo(9,1980,1000)
              mechdog.set_servo(10,1800,1000)
              time.sleep(1.5)
      else:
        time.sleep(0.03)
